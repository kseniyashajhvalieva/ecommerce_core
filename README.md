...
- Определяет обязательные методы `__init__` и `__str__`
...
При попытке создания товара с `quantity == 0` вызывается исключение `ValueError` с сообщением:
*`«Товар с нулевым количеством не может быть добавлен»`*.

#### Метод `middle_price`
Новый метод класса `Category` для расчёта средней цены товаров в категории:
* Суммирует цены всех товаров в категории
* Делит на количество товаров
* При отсутствии товаров (деление на ноль) возвращает `0`
* Использует обработку исключений `try/except ZeroDivisionError`
