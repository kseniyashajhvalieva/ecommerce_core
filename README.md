...
Наследует `Product`. Добавлены атрибуты:
* `efficiency` (float) — производительность
* `model` (str) — модель
* `memory` (int) — объем памяти
* `color` (str) — цвет

#### Класс `LawnGrass` (Трава газонная)
Наследует `Product`. Добавлены атрибуты:
...
#### Ограничения
* Сложение (`__add__`) разрешено **только для объектов одного класса** (иначе `TypeError`).
* В категорию можно добавлять **только объекты `Product` или его наследников** (проверка `isinstance`).
